...
export default function Layout({ children, className, id, title }) {
  ...
  return (
    <section
      id={id}
      className={`${className} w-full min-h-screen py-24 px-32 relative dark:bg-darkBg`}
    >
      <div className="py-2 mx-auto flex flex-col items-center justify-center text-center">
        <h1 className="dark:text-white text-8xl font-bold w-full capitalize mb-16 text-center tracking-[1rem]">
          {title}
        </h1>
      </div>
      {showBtn && (
        <motion.button
          ...
          className="fixed right-[2%] bottom-[5%] font-3xl rounded-full dark:text-white"
